...
const START_WORK_REVIEW_AND_CODING_STATUSES = [...REVIEW_INPUT_STATUSES, ...CODING_INPUT_STATUSES];
...
function stableOrbitSkillPath(skillName = DEFAULT_PACKAGED_SKILL) {
    return path.join(stableOrbitSkillDir(skillName), 'SKILL.md');
}
function stableOrbitSkillDir(skillName = DEFAULT_PACKAGED_SKILL) {
    return path.join(homeDir(), '.orbit', 'skills', skillName);
}
function bundledSkillsDir() {
    return path.join(cliPackageRoot(), 'skills');
}
function bundledSkillDir(skillName = DEFAULT_PACKAGED_SKILL) {
    return path.join(bundledSkillsDir(), skillName);
}
function bundledOrbitSkillPath(skillName = DEFAULT_PACKAGED_SKILL) {
    return path.join(bundledSkillDir(skillName), 'SKILL.md');
}
function agentSkillDir(agent, skillName = DEFAULT_PACKAGED_SKILL) {
    if (agent === 'codex')
...
function agentSkillPath(agent, skillName = DEFAULT_PACKAGED_SKILL) {
...
function omitUndefinedValues(values) {
...
const sessionMachineMetadataSyncs = new Set();
async function syncCurrentSessionMachineMetadataOnce(backendUrl, token) {
    const safeToken = safeString(token);
    if (!safeToken)
        return;
    const key = `${normalizeBackendUrl(backendUrl)}\n${safeToken}`;
    if (sessionMachineMetadataSyncs.has(key))
        return;
    await syncCurrentSessionMachineMetadata(backendUrl, safeToken);
    sessionMachineMetadataSyncs.add(key);
}
async function syncCurrentSessionMachineMetadata(backendUrl, token) {
    try {
...
async function skillDirectoriesIdentical(sourceDir, targetDir) {
...
async function copySkillBundleIfAllowed(sourceDir, targetDir, force) {
    ensureDir(path.dirname(targetDir));
    if (existsSync(targetDir)) {
        if (await skillDirectoriesIdentical(sourceDir, targetDir))
            return 'identical';
        if (!force) {
...
function workspaceProjectMatchesFilter(product, project, filter) {
    if (filter.productLineId && filter.productLineId !== product.id)
        return false;
    if (filter.productLineUuid && filter.productLineUuid !== product.uuid)
        return false;
...
function describeWorkspaceFilter(filter) {
    const parts = [
...
async function resolveAxisWorkspaceForWorker() {
    ...
    for (const productDetail of details) {
        ...
        for (const project of productDetail.modules) {
            if (!workspaceProjectMatchesFilter(productDetail.product, project, filter))
                continue;
...
function parseCreateEmployeeLanguageArg(value) {
    const normalized = value?.trim().toLowerCase();
    if (!normalized)
        return null;
...
async function resolveCreateEmployeeLanguage() {
    ...
    if (explicit)
        return explicit;
    if (!process.stdin.isTTY || hasFlag('--json'))
        return 'zh';
    const prompt = await createPromptSession();
...
async function fetchStartWorkRemoteEmployees(backendUrl, tokenHint) {
...
async function resolveStartWorkAllEmployees(backendUrl, tokenHint) {
    const warnings = [];
    if (backendUrl) {
        try {
            ...
            if (activeEmployees.length > 0)
                return { employees: activeEmployees, warnings };
...
function buildEmployeeSoulPrompt(employeeId, agent, language) {
...
function fallbackEmployeeSoul(employeeId, agent, language, warning) {
...
function initialEmployeeSkill(employeeId, agent, language) {
    if (language === 'zh') {
        return [
            '# Axis 员工技能',
            '',
...
function initialEmployeeMemory(employeeId, language) {
    if (language === 'zh') {
        return [
            '# Axis 员工记忆',
            '',
...
function normalizeEmployeeNameCandidate(value) {
    return value.trim().replace(/^["'“”‘’]+|["'“”‘’]+$/g, '').trim();
}
function isMeaninglessEmployeeName(name, employeeId) {
    const normalized = name.trim();
    if (!normalized)
        return true;
    const lower = normalized.toLowerCase();
    if (lower === employeeId.toLowerCase())
        return true;
    if (lower.startsWith('emp_'))
        return true;
    if (/^agent\s*\d*$/i.test(normalized))
        return true;
    if (/^axis\s+employee(?:\s+[a-z0-9_-]+)?$/i.test(normalized))
        return true;
    if (/^(employee|worker|assistant|unknown|unnamed)$/i.test(normalized))
        return true;
    if (/^[a-z0-9_-]{8,}$/i.test(normalized) && /[0-9_-]/.test(normalized))
        return true;
    return false;
}
function isHumanizedEmployeeName(name, employeeId, language) {
    if (isMeaninglessEmployeeName(name, employeeId))
        return false;
    if (language === 'zh') {
...
function extractEmployeeDisplayName(markdown, employeeId, language) {
    const candidates = [];
...
function employeeSoulMatchesLanguage(markdown, language) {
...
async function ensureEmployeeRuntimeFiles(employeeId, agent, language, soul) {
...
function axisEmployeeWorkContextPath(employeeId) {
    return path.join(axisEmployeeDir(employeeId), 'work-context.json');
}
function axisEmployeeRoleWorkflowPath(employeeId) {
    return path.join(axisEmployeeDir(employeeId), 'role-workflow.json');
}
function employeeWorkContextFromHeartbeatResponse(response) {
    if (!isJson(response) || !isJson(response.workContext))
        return null;
    return {
...
function employeeDocumentCacheEntry(raw, kind) {
    const parsed = remoteEmployeeDocumentContent(raw);
    if (!parsed?.found)
        return null;
    if (isJson(raw)) {
...
async function syncEmployeeWorkContextFromHeartbeat(values) {
    ...
    const employee = isJson(context.employee) ? context.employee : {};
    const rawDocuments = isJson(context.documents) ? context.documents : {};
    ...
    const roleWorkflow = isJson(context.roleWorkflow) ? context.roleWorkflow : null;
    if (roleWorkflow) {
    ...
    const agent = safeString(employee.agentType) ?? safeString(employee.agent) ?? safeString((await readJsonFile(path.join(employeeDir, 'config.json'), {})).agentType) ?? 'codex';
    const cachedContext = {
        employeeId,
        backendUrl: normalizeBackendUrl(values.backendUrl),
    ...
    const currentConfig = await readJsonFile(path.join(employeeDir, 'config.json'), {});
...
async function readSyncedEmployeeWorkContextDocuments(employeeId) {
    const contextPath = axisEmployeeWorkContextPath(employeeId);
    if (!existsSync(contextPath))
        return null;
    const payload = await readJsonFile(contextPath, {});
    if (!isJson(payload.documents) && !isJson(payload.roleWorkflow) && !isJson(payload.employee))
        return null;
...
function parseStartWorkAgentArg(value) {
    if (!value)
        return null;
    if (value === 'codex' || value === 'claude-code' || value === 'claude')
        return value;
    if (value === 'cc')
        return 'claude-code';
    throw new Error('--agent must be one of: codex, claude-code, claude');
}
function startWorkRunnerAgent(agent) {
    return agent === 'codex' ? 'codex' : 'claude-code';
}
function startWorkAgentCommand(agent) {
    return agent === 'codex' ? 'codex' : 'claude';
}
function startWorkAgentName(agent) {
    return agent === 'codex' ? 'Codex' : 'Claude Code';
}
function normalizeConfiguredStartWorkAgent(value) {
    if (value === 'codex' || value === 'claude-code')
        return value;
    return null;
}
async function resolveStartWorkAgent(repoPath) {
...
function cliVersion() {
    try {
        const pkg = JSON.parse(readFileSync(path.join(cliPackageRoot(), 'package.json'), 'utf8'));
...
async function linuxClockTicksPerSecond() {
...
async function linuxBootTimeSeconds() {
    try {
...
async function processStartMillis(pid) {
    if (process.platform !== 'linux')
        return null;
    try {
        ...
        if (!Number.isFinite(startTicks) || bootSeconds === null)
            return null;
...
function commandLineLooksLikeWorker(commandLine, sessionId) {
    return commandLine.includes('start-work') && commandLine.includes(sessionId);
}
async function workerProcessStatus(sessionId, pid, state, config) {
...
async function writeWorkerState(sessionId, state) {
...
function compactLastHeartbeat(value) {
...
function emitStartWorkProgress(message) {
...
function startWorkHeartbeatTargetFromBinding(repoPath, binding) {
    return {
        repoPath,
        binding,
        productLineId: binding.productLineId ?? binding.productLineUuid ?? null,
...
async function inferStartWorkHeartbeatTarget(repoPath) {
    if (repoPath) {
        const binding = await readProjectBinding(repoPath);
    ...
    if (!token && !safeString(config.backendUrl) && !getArg('--backend-url'))
        return null;
...
function startWorkTargetsScope(targets) {
    return {
...
function requestedStartWorkScope() {
...
function projectBindingMatchesStartWorkFilters(binding) {
    const projectId = getArg('--project-id');
    const projectUuid = getArg('--project-uuid');
    const productLineId = getArg('--product-line-id');
    const productLineUuid = getArg('--product-line-uuid');
    if (projectId && projectId !== binding.projectId && projectId !== binding.projectUuid)
        return false;
    if (projectUuid && projectUuid !== binding.projectUuid)
        return false;
    if (productLineId && productLineId !== binding.productLineId && productLineId !== binding.productLineUuid)
        return false;
    if (productLineUuid && productLineUuid !== binding.productLineUuid)
        return false;
    return true;
}
function startWorkTargetFromBinding(repoPath, binding) {
    const projectId = projectApiId(binding);
    if (!projectId)
        return null;
    return {
...
async function resolveStartWorkTargets() {
    const repoArg = getArg('--repo');
    if (repoArg) {
        const repoPath = path.resolve(repoArg);
        const binding = await readProjectBinding(repoPath);
...
function fallbackAgentContextDocument(key, warning, source = 'project') {
...
function startWorkContextDocumentsFromPayload(payload) {
    ...
    for (const key of keys) {
        const raw = isJson(rawDocuments[key]) ? rawDocuments[key] : null;
        if (!raw) {
...
function startWorkRemoteEmployeeDocumentsRoot(payload) {
    if (!isJson(payload))
        return {};
    if (isJson(payload.workContext) && isJson(payload.workContext.documents))
        return payload.workContext.documents;
    if (isJson(payload.context) && isJson(payload.context.documents))
        return payload.context.documents;
    if (isJson(payload.documents))
        return payload.documents;
    if (isJson(payload.employee) && isJson(payload.employee.documents))
        return payload.employee.documents;
    return {};
}
...
function startWorkRemoteEmployeeRoleWorkflow(payload) {
    if (!isJson(payload))
        return null;
    if (isJson(payload.workContext) && isJson(payload.workContext.roleWorkflow))
        return payload.workContext.roleWorkflow;
    if (isJson(payload.context) && isJson(payload.context.roleWorkflow))
        return payload.context.roleWorkflow;
    if (isJson(payload.employee) && isJson(payload.employee.roleWorkflow))
        return payload.employee.roleWorkflow;
    if (isJson(payload.roleWorkflow))
        return payload.roleWorkflow;
    return null;
}
...
function roleWorkflowStepsMarkdown(workflow) {
    const steps = Array.isArray(workflow.steps) ? workflow.steps : [];
    if (steps.length === 0)
        return ['Steps: none configured.'];
    const lines = ['Steps:'];
    for (const step of steps) {
        ...
        const dependsOn = Array.isArray(step.dependsOn)
            ? step.dependsOn.map((entry) => safeString(entry)).filter((entry) => Boolean(entry))
            : [];
        if (role)
...
function startWorkEmployeeRoleWorkflowDocument(workflow) {
...
function startWorkEmployeeContextDocumentsFromPayload(employeeId, payload) {
    const keys = [
...
async function fetchStartWorkContext(target) {
...
function hubMissingAgentContextWarning(warning) {
    return /Agent context document (?:soul|skill|memory)\.md was not found; using empty fallback\./.test(warning);
}
async function fetchStartWorkEmployeeContext(target, employeeId) {
    const token = await tokenForBinding(target.binding);
    try {
...
async function preloadStartWorkContexts(targets, progress) {
    ...
    for (const target of targets) {
        ...
        if (employeeId)
            progress(`context: fetched remote employee documents for ${employeeId}`);
...
function startWorkContextMarkdown(documents) {
    return documents
        .map((document) => {
...
function startWorkContextDocumentsForSource(documents, source) {
    return documents.filter((document) => document.source === source);
}
function startWorkResponsibilityContextDocuments(documents) {
...
function startWorkEmployeeContextUnavailable(documents) {
    const employeeDocuments = startWorkContextDocumentsForSource(documents, 'employee');
    return employeeDocuments.length > 0 && employeeDocuments.every((document) => document.found !== true);
}
function startWorkPromptContextSections(documents, fallbackTitle) {
    const employeeDocuments = startWorkContextDocumentsForSource(documents, 'employee');
...
function startWorkCandidateSummary(item) {
    return {
        id: workItemId(item),
...
function startWorkSelectionJson(selection) {
    return {
...
function parseStartWorkSelectionOutput(output, candidateIds) {
...
function keywordIsNegated(text, index) {
...
function keywordHits(text, keywords, options = {}) {
    const hits = [];
    ...
    for (const keyword of keywords) {
        ...
        while ((match = pattern.exec(normalized)) !== null) {
            if (options.ignoreNegated && keywordIsNegated(normalized, match.index))
                continue;
            hits.push(keyword);
            break;
        }
...
function startWorkCandidateSearchText(summary) {
    return [
...
function startWorkCandidateStatusesForEmployeeRole(role) {
    if (role === 'product')
        return REVIEW_INPUT_STATUSES;
    if (role === 'architecture' || role === 'design')
        return START_WORK_REVIEW_AND_CODING_STATUSES;
    return CODING_INPUT_STATUSES;
}
function startWorkCandidateQueueLabel(statuses) {
    const canonical = Array.from(new Set(statuses.map((status) => canonicalLifecycleStatus(status))));
    if (canonical.length === 1)
        return canonical[0] ?? 'unknown';
...
async function selectStartWorkItem(values) {
    ...
    const candidateIds = new Set(candidates.map((candidate) => safeString(candidate.id)).filter((id) => Boolean(id)));
    ...
    try {
        ...
        if (parsed.selection) {
            if (parsed.selection.selectedWorkItemId === null) {
                ...
                if (fallback.selectedWorkItemId)
                    return fallback;
            }
...
async function claimStartWorkItem(target, item, payload) {
    const id = workItemId(item);
    if (!id)
        return { claimed: false, conflict: false, response: null, warning: 'WAIT_CODE WorkItem had no id and could not be claimed.' };
    try {
...
async function postWorkItemLifecycle(target, workItemID, action, payload) {
    try {
...
async function patchStartWorkResult(target, workItemID, values) {
...
async function refreshStartWorkTarget(target) {
    if (!target.repoPath)
        return target;
    ...
    if (!refreshed)
        return startWorkHeartbeatTargetFromBinding(target.repoPath, binding);
    return refreshed;
}
async function processStartWorkTarget(values) {
    ...
    if (selection.warning)
        pushUniqueWarning(summary.warnings, selection.warning);
...
async function sendStartWorkHeartbeat(values) {
    ...
    try {
        ...
        retryState.lastSuccessAt = payload.sentAt;
...
function heartbeatRetryDelayMs(intervalSeconds, failures) {
...
function shouldLogHeartbeatFailure(retryState, warning) {
    if (retryState.failures === 1)
        return true;
    if (retryState.failures % 5 === 0)
        return true;
...
function startStartWorkHeartbeatLoop(values) {
    ...
    let inFlight = null;
    const retryState = { failures: 0, lastWarning: null, lastSuccessAt: null };
    const send = async () => {
        if (!currentTarget)
            return;
...
function startWorkErrorMessage(error) {
...
async function runStartWorkForeground(options) {
    ...
    let context = { contexts: new Map(), warnings: [] };
...
function backgroundStartWorkArgs(values) {
    ...
    if (productLineId)
        args.push('--product-line-id', productLineId);
    if (productLineUuid)
        args.push('--product-line-uuid', productLineUuid);
    if (values.maxIterations !== null)
        args.push('--iterations', String(values.maxIterations));
    return args;
}
...
function printStartWorkResult(payload) {
    ...
    if (safeString(payload.logPath))
        console.log(`log: ${payload.logPath}`);
    if (safeString(payload.stopReason))
...
async function workStatusCommand() {
...
function gateReviewArtifactForUserConfirmation(artifact) {
    return {
        ...artifact,
        status: LIFECYCLE_WAIT_USER_CONFIRM,
        workItems: artifact.workItems.map((item) => (isJson(item)
...
function poolKindFromDocument(document) {
    const source = isJson(document.source) ? document.source : {};
...
function normalizeDocumentReviewCandidate(document) {
    if (!isReviewInputStatus(safeString(document.status)))
        return null;
...
function reviewCandidateSourceCounts(seeds, workItems, documents, candidates, duplicates) {
    return {
...
async function discoverReviewCandidates(binding, projectId, options = {}) {
    ...
    return {
        ...
        warnings: [optionalPoolSeedProbeWarning(seedProbe.warning), workItemProbe.warning, documentProbe.warning].filter((warning) => Boolean(warning)),
    };
}
...
function isPoolSeedReviewCandidate(candidate) {
...
async function markReviewWorkItemHandled(repoPath, candidate, submit) {
    ...
    for (const workItemIdValue of workItemIds) {
        try {
            ...
            if (relinkSourceWorkItems && documentId)
                payload.sourceArtifactId = documentId;
