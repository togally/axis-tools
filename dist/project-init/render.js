...
const schemaPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../schemas/project-init-answers.schema.json');
const answersSchema = JSON.parse(readFileSync(schemaPath, 'utf8'));
...
function assertSafeDecisionValue(field, answer) {
...
function assertDecisionValue(field, answer) {
...
function assertExactInspectionMetadata(inspection, answers) {
    if (answers.repo !== inspection.repo)
        throw new Error('answers repository does not match inspection');
...
export function validateAnswers(inspection, input) {
    ...
    answers.decisions.forEach((answer, index) => {
        if (seen.has(answer.key))
            throw new Error(`duplicate project-init answer decision: ${answer.key}`);
        seen.add(answer.key);
...
function documentMap(document, label) {
    if (!document.contents)
        document.set('schema', undefined);
...
function valueAt(document, pathSegments) {
    return document.getIn(pathSegments);
}
function setIfDifferent(document, pathSegments, value) {
    if (!sameValue(valueAt(document, pathSegments), value))
        document.setIn(pathSegments, structuredClone(value));
}
function setMapIfDifferent(map, key, value) {
    if (!sameValue(map.get(key), value))
        map.set(key, structuredClone(value));
}
function deleteIfPresent(document, pathSegments) {
    if (document.getIn(pathSegments) !== undefined)
        document.deleteIn(pathSegments);
}
function finalValues(inspection, answers) {
    return new Map(inspection.fields.map((field, index) => [field.key, answers.decisions[index].value]));
}
function sourceValue(sourceFiles, key) {
    const aliases = {
    ...
    const value = candidate ? sourceFiles[candidate] : null;
    return typeof value === 'string' ? value : null;
}
function selectedMapNode(collection, identityKey, identityValue) {
    if (isSeq(collection)) {
...
function collectionNode(parent, key, label) {
    ...
    if (!isSeq(existing) && !isMap(existing))
        throw new Error(`${label}.${key} must be a sequence or mapping`);
    return existing;
}
function addRecord(collection, identityKey, identityValue, values) {
    const record = new YAMLMap();
    for (const [key, value] of Object.entries(values)) {
        if (Array.isArray(value)) {
            const sequence = new YAMLSeq();
            for (const item of value)
                sequence.add(structuredClone(item));
...
function upsertRecord(collection, identityKey, identityValue, values) {
...
function renderMainConfig(values, sourceText) {
    ...
    for (const key of legacyOssFields)
        deleteIfPresent(document, ['oss', key]);
    return document.toString();
}
function updateRegistry(inspection, values, sourceText) {
    ...
    for (const [key, fieldKey] of [
        ...
        const value = values.get(fieldKey);
        if (value !== null && value !== undefined)
            setMapIfDifferent(organization, key, value);
    }
    ...
    for (const key of ['provider', 'bucket', 'prefix', 'endpoint_env', 'region_env', 'access_key_id_env', 'access_key_secret_env', 'security_token_env']) {
        ...
        if (value !== null && value !== undefined)
            setMapIfDifferent(profile, key, value);
    }
    ...
    const matches = isSeq(projects)
        ...
        : isMap(projects) && mapNode(projects.get(projectSlug, true))
            ...
            : [];
    if (matches.length > 1)
        throw new Error(`duplicate project slug in organization: ${projectSlug}`);
...
function renderLocalConfig(inspection, values, sourceText) {
    if (sourceText === null)
        return null;
    ...
    const authorizedRemovals = new Set(inspection.fields
        .filter((field) => field.resolution === 'remove' || field.disposition === 'remove')
        .map((field) => field.key));
    for (const field of inspection.fields) {
        if (!authorizedRemovals.has(field.key))
            continue;
        if (values.get(field.key) !== null)
            throw new Error(`unconfirmed removal field cannot be rendered: ${field.key}`);
        deleteIfPresent(document, field.key.split('.'));
    }
    return document.toString();
}
function renderGitignore(sourceText) {
...
export function renderProjectFiles(options) {
    ...
    return {
        main_config: renderMainConfig(values, sourceValue(options.sourceFiles, 'main_config')),
        local_config: renderLocalConfig(options.inspection, values, sourceValue(options.sourceFiles, 'local_config')),
        target_registry: updateRegistry(options.inspection, values, sourceValue(options.sourceFiles, 'target_registry')),
