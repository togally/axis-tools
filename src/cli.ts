...
const EMPLOYEE_CONTEXT_DOCUMENTS = ['soul', 'skill', 'memory'] as const;
type EmployeeContextDocumentKind = typeof EMPLOYEE_CONTEXT_DOCUMENTS[number];
...
interface TeamIntakeSubmitResult {
  ok: boolean;
  mode: 'team-intake' | 'local-seed' | 'dry-run';
  repo: string;
  pool: string;
  kind: PoolConfig['kind'];
  title: string;
...
interface StartWorkRemoteEmployee {
  id: string;
  name?: string | null;
  status?: string | null;
  role?: string | null;
...
function printStopWorkUsage(): void {
...
function printSyncEmployeeUsage(): void {
...
async function readCurrentProductLineBinding(): Promise<DiscoveredProductLineBinding | null> {
...
async function fetchAccountPlan(backendUrl: string, token: string): Promise<Json> {
...
function printAccountPlan(payload: Json): void {
...
function orbitErrorDetailFromBody(body: string): string | null {
...
async function orbitHttpErrorFromResponse(response: Response, url: string): Promise<OrbitHttpError> {
...
type SyncRepositoryResult = {
  status: 'cloned' | 'pulled' | 'existing-git' | 'skipped-nonempty' | 'clone-failed';
  error?: string;
  backupPath?: string;
};
...
async function withRepositorySyncLock<T>(targetPath: string, run: () => Promise<T>): Promise<T> {
...
async function moveNonGitWorkspaceAside(targetPath: string): Promise<string> {
...
function temporaryClonePath(targetPath: string): string {
  const suffix = `${Date.now().toString(36)}-${randomBytes(3).toString('hex')}`;
  return `${targetPath}.axis-clone-${suffix}`;
}

function httpsCloneFallback(repoUrl: string): string | null {
...
function cloneUrlCandidates(repoUrl: string): string[] {
  const fallback = httpsCloneFallback(repoUrl);
  return fallback && fallback !== repoUrl ? [repoUrl, fallback] : [repoUrl];
}

async function cloneRepositoryWithFallback(repoUrl: string, targetPath: string): Promise<GitCloneResult> {
  const errors: string[] = [];
...
async function syncRepository(repoUrl: string, targetPath: string, options: SyncRepositoryOptions = {}): Promise<SyncRepositoryResult> {
  return withRepositorySyncLock(targetPath, () => syncRepositoryUnlocked(repoUrl, targetPath, options));
}

async function syncRepositoryUnlocked(repoUrl: string, targetPath: string, options: SyncRepositoryOptions): Promise<SyncRepositoryResult> {
  ...
  if (await isGitRepo(targetPath)) {
    if (options.pullExisting === false) return { status: 'existing-git' };
...
function shouldScanPoolSeedDir(entryName: string): boolean {
  ...
  if (entryName.includes('.axis-non-git-') || entryName.includes('.axis-clone-') || entryName.endsWith('.axis-sync.lock')) return false;
...
function normalizeSubmitKind(value: string | null): PoolConfig['kind'] | null {
  const normalized = value?.trim().toLowerCase();
  if (!normalized) return null;
  if (normalized === 'requirement' || normalized === 'req') return 'requirement';
  if (normalized === 'bug' || normalized === 'defect') return 'bug';
  if (normalized === 'suggestion' || normalized === 'sug' || normalized === 'improvement' || normalized === 'optimization') return 'suggestion';
  if (normalized === 'idea' || normalized === 'ide') return 'idea';
  throw new Error('--type must be one of: requirement, bug, suggestion, idea');
}

function inferSubmitKind(text: string): PoolConfig['kind'] {
  if (/bug|crash|error|exception|fail|failed|报错|错误|异常|失败|崩溃|无法|500|404/i.test(text)) return 'bug';
  if (/优化|建议|体验|改进|improve|suggest|enhance/i.test(text)) return 'suggestion';
  if (/想法|灵感|idea|maybe/i.test(text)) return 'idea';
  return 'requirement';
}

function submitKindFromArgs(text: string): PoolConfig['kind'] {
  return normalizeSubmitKind(getArg('--type') ?? getArg('--kind')) ?? inferSubmitKind(text);
}

function buildTeamIntakePayload(kind: PoolConfig['kind'], seed: string, repoPath: string): Json {
...
async function submitTeamIntakeToHub(binding: ProjectBinding, token: string | null, payload: Json): Promise<unknown> {
  const projectId = projectApiId(binding);
  if (!projectId) throw new Error('project binding has no projectId/projectUuid');
  if (!token) throw new Error('project binding has no token and no cached login session');
  return postOrbitJson(
    binding.backendUrl,
    `/api/projects/${encodeURIComponent(projectId)}/team-intake`,
    payload,
    token,
  );
}

function extractTeamIntakeWorkItemId(payload: unknown): string | null {
  if (!isJson(payload)) return null;
...
function extractTeamIntakeDocumentId(payload: unknown): string | null {
  if (!isJson(payload)) return null;
...
function extractTeamIntakeConversationId(payload: unknown): string | null {
  if (!isJson(payload)) return null;
...
async function submitTeamIntake(repoPath: string, seed: string): Promise<TeamIntakeSubmitResult> {
...
function printTeamIntakeSubmit(result: TeamIntakeSubmitResult): void {
  if (hasFlag('--json')) {
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  console.log(`${result.kind}: ${result.title}`);
  console.log(`mode: ${result.mode}`);
  if (result.teamId) console.log(`team: ${result.teamId}`);
  if (result.targetEmployeeId) console.log(`employee: ${result.targetEmployeeId}`);
  if (result.id) console.log(`workItem: ${result.id}`);
  if (result.documentId) console.log(`document: ${result.documentId}`);
  if (result.conversationId) console.log(`conversation: ${result.conversationId}`);
  if (result.url) console.log(`url: ${result.url}`);
  if (result.savedPath) console.log(`savedPath: ${result.savedPath}`);
  if (result.warning) console.log(`warning: ${result.warning}`);
}
...
async function submitCommand(args: string[]): Promise<void> {
  ...
  if (!input.trim()) throw new Error('submit text is required');
...
function axisEmployeeDocumentPath(employeeId: string, key: EmployeeContextDocumentKind): string {
  return path.join(axisEmployeeDir(employeeId), `${key}.md`);
}

function axisEmployeeAgentsPath(employeeId: string): string {
  return path.join(axisEmployeeDir(employeeId), 'AGENTS.md');
}

function axisEmployeeClaudePath(employeeId: string): string {
...
function localEmployeeConfigMatchesBackend(employee: AxisCurrentEmployee, backendUrl: string | null): boolean {
  if (!backendUrl) return true;
  if (!employee.backendUrl) return false;
  return normalizeBackendUrl(employee.backendUrl) === normalizeBackendUrl(backendUrl);
}

async function readLocalEmployees(backendUrl: string | null): Promise<AxisCurrentEmployee[]> {
  let entries: string[] = [];
  try {
  ...
  for (const entry of entries.sort()) {
    ...
    if (employee && localEmployeeConfigMatchesBackend(employee, backendUrl)) employees.push(employee);
  }

  return employees;
}

let startWorkEmployeeResolutionWarnings: string[] = [];
const emittedStartWorkEmployeeResolutionWarnings = new Set<string>();

function pushStartWorkEmployeeResolutionWarning(warning: string): void {
  pushUniqueWarning(startWorkEmployeeResolutionWarnings, warning);
}

function reportStartWorkEmployeeResolutionWarnings(summary: StartWorkSummary, progress?: (message: string) => void): void {
...
function remoteEmployeesRoot(payload: unknown): unknown[] {
  if (!isJson(payload)) return [];
  if (Array.isArray(payload.employees)) return payload.employees;
  if (Array.isArray(payload.items)) return payload.items;
  if (Array.isArray(payload.data)) return payload.data;
  if (isJson(payload.data)) return remoteEmployeesRoot(payload.data);
  return [];
}

function asStartWorkRemoteEmployee(value: unknown): StartWorkRemoteEmployee | null {
  if (!isJson(value)) return null;
...
function startWorkRemoteEmployeeSelectable(employee: StartWorkRemoteEmployee): boolean {
  const status = employee.status?.trim().toLowerCase();
  if (employee.archived || employee.deleted || employee.archivedAt || employee.deletedAt) return false;
  return status !== 'archived' && status !== 'deleted';
}

function startWorkRemoteEmployeeActive(employee: StartWorkRemoteEmployee): boolean {
  const status = employee.status?.trim().toLowerCase();
  return employee.active === true || status === 'active';
}

function chooseStartWorkRemoteEmployee(employees: StartWorkRemoteEmployee[], backendUrl: string): string | null {
  ...
  pushStartWorkEmployeeResolutionWarning(
    `Found multiple remote Axis employees active at ${normalizeBackendUrl(backendUrl)} and could not choose one automatically. Run axis create-employee to select the intended current employee, set a default employee, or pass --employee-id <id>.`,
  );
  return null;
}

async function tokenForStartWorkBackend(backendUrl: string, tokenHint?: string | null): Promise<string | null> {
...
async function persistResolvedStartWorkEmployee(employee: StartWorkRemoteEmployee, backendUrl: string, agentHint: string | null): Promise<void> {
...
async function resolveStartWorkRemoteEmployeeId(backendUrl: string | null, tokenHint?: string | null, agentHint?: string | null): Promise<string | null> {
  ...
  try {
    ...
    if (employee) await persistResolvedStartWorkEmployee(employee, backendUrl, safeString(agentHint));
    return employeeId;
  } catch (error) {
...
function productLineBindingToProjectBinding(rootPath: string, binding: ProductLineBinding): ProjectBinding {
  const owner = binding.owner ?? binding.account ?? binding.user?.account ?? null;
  const projectBinding: ProjectBinding = {
    backendUrl: binding.backendUrl,
...
function productLineLoginSession(binding: ProductLineBinding): OrbitLoginSession {
  return {
    token: binding.token,
    key: binding.key,
    session: binding.session,
...
function projectBindingWithProductLineFallback(binding: ProjectBinding, productLine: ProductLineBinding): ProjectBinding {
  return {
    ...binding,
...
function projectBindingBelongsToProductLine(binding: ProjectBinding, productLine: ProductLineBinding): boolean {
  if (normalizeBackendUrl(binding.backendUrl) !== normalizeBackendUrl(productLine.backendUrl)) return false;
  ...
  if (projectProductLineId && rootProductLineId && projectProductLineId === rootProductLineId) return true;
  if (projectProductLineId && rootProductLineUuid && projectProductLineId === rootProductLineUuid) return true;
  if (projectProductLineUuid && rootProductLineUuid && projectProductLineUuid === rootProductLineUuid) return true;
  if (projectProductLineUuid && rootProductLineId && projectProductLineUuid === rootProductLineId) return true;
  return !projectProductLineId && !projectProductLineUuid;
}

function productLineFromBinding(binding: ProductLineBinding): OrbitProductLine {
  return {
...
function projectModuleFromBinding(repoPath: string, binding: ProjectBinding): OrbitProjectModule {
...
async function syncProductLineBindingProject(repoPath: string, binding: ProjectBinding): Promise<{
  binding: ProjectBinding;
  materialized: boolean;
  repoUrl: string | null;
  syncStatus: string;
...
async function resolveProductLineWorkspaceForWorker(productLine: DiscoveredProductLineBinding): Promise<AxisWorkspaceResolution> {
  return withRepositorySyncLock(productLine.rootPath, () => resolveProductLineWorkspaceForWorkerUnlocked(productLine));
}

async function resolveProductLineWorkspaceForWorkerUnlocked(productLine: DiscoveredProductLineBinding): Promise<AxisWorkspaceResolution> {
  ...
  for (const candidate of discovery.projects) {
    if (!projectBindingBelongsToProductLine(candidate.binding, binding)) continue;
    const projectBinding = projectBindingWithProductLineFallback(candidate.binding, binding);
    if (!projectBindingMatchesStartWorkFilters(projectBinding)) continue;
...
async function startWorkBackendHint(repoPath: string | null): Promise<{ backendUrl: string | null; token: string | null }> {
  if (repoPath) {
    const binding = await readProjectBinding(repoPath);
    if (binding?.backendUrl) return { backendUrl: normalizeBackendUrl(binding.backendUrl), token: safeString(binding.token) };
  }
  ...
  if (explicitBackendUrl) return { backendUrl: normalizeBackendUrl(explicitBackendUrl), token: null };
  const config = await readGlobalOrbitConfig();
  return {
    backendUrl: safeString(config.backendUrl) ? normalizeBackendUrl(safeString(config.backendUrl) as string) : null,
...
function syncEmployeePayloadEmployee(payload: unknown): Json {
  if (isJson(payload) && isJson(payload.employee)) return payload.employee as Json;
  return isJson(payload) ? payload : {};
}

function syncEmployeeDocumentFromPayload(employee: Json, kind: EmployeeContextDocumentKind): { content: string; version: number | null; updatedAt: string | null } {
  const documents = isJson(employee.documents) ? employee.documents as Json : {};
...
function compactStringObject(entries: Array<[string, string | null | undefined]>): Json | null {
  const payload: Json = {};
...
function employeeProfileFromPayload(employee: Json): Json | null {
...
function employeeModelFromPayload(employee: Json): Json | null {
  return compactStringObject([
...
function employeeRuntimeModelFromPayload(employee: Json): Json | null {
  return compactStringObject([
...
function runtimeModelForHeartbeat(config: Json): Json | null {
  const model = isJson(config.model) ? config.model as Json : {};
  const runtimeModel = isJson(config.runtimeModel) ? config.runtimeModel as Json : {};
  return compactStringObject([
...
async function readStartWorkEmployeeRuntimeModel(employeeId: string | null): Promise<Json | null> {
  if (!employeeId) return null;
  return runtimeModelForHeartbeat(await readJsonFile<Json>(path.join(axisEmployeeDir(employeeId), 'config.json'), {}));
}

function runtimeModelFields(model: Json | null): Json {
  if (!model) return {};
  return {
...
function modelFromHeartbeat(value: Json | null): Json | null {
  if (!value) return null;
  return compactStringObject([
...
function modelSummaryText(model: Json | null): string | null {
  const parts = [safeString(model?.provider), safeString(model?.name), safeString(model?.level)].filter((value): value is string => Boolean(value));
...
function claudeWorkspaceInstructions(): string {
  return [
...
async function writeEmployeeWorkspaceGuides(employeeId: string, values: { name: string | null; role: string | null; backendUrl: string }): Promise<void> {
...
async function pushLocalEmployeeDocuments(values: { backendUrl: string; token: string | null; employeeId: string }): Promise<string[]> {
  const pushed: string[] = [];
...
async function syncEmployeeWorkspace(values: {
  ...
  const documentResults: Json[] = [];
  const syncDocuments: Json = {};
...
async function syncEmployeeCommand(): Promise<void> {
  ...
  if (Array.isArray(payload.pushed) && payload.pushed.length > 0) console.log(`pushed: ${payload.pushed.join(', ')}`);
}
...
async function resolveStartWorkAgent(repoPath: string | null): Promise<StartWorkAgentChoice> {
  ...
  if (productLineAgent) return productLineAgent;
...
function workerEmployeeId(state: Json, config: Json, lastHeartbeat: Json | null): string | null {
...
async function inferStartWorkHeartbeatTarget(repoPath: string | null): Promise<StartWorkHeartbeatTarget | null> {
  ...
  const productLine = await readCurrentProductLineBinding();
  if (productLine) {
    return startWorkHeartbeatTargetFromBinding(
      productLine.rootPath,
      productLineBindingToProjectBinding(productLine.rootPath, productLine.binding),
    );
  }
...
function buildStartWorkAgentPrompt(values: {
  ...
  return [
    ...
    `Employee workspace: ${startWorkEmployeeId() ? axisEmployeeDir(startWorkEmployeeId() as string) : 'unassigned'}`,
...
async function patchStartWorkResult(target: StartWorkTarget, workItemID: string, values: {
  ...
  const payload: Json = { notes, owner: values.sessionId };
  if (!values.ok) payload.status = values.status ?? 'WAIT_CODE';
  try {
...
function detectStartWorkAgentBlockedOutput(output: string): string | null {
  const text = output.replace(/\s+/g, ' ').trim();
  if (!text) return null;
  const checks: { reason: string; pattern: RegExp }[] = [
...
async function releaseStartWorkItemAfterFailure(target: StartWorkTarget, workItemID: string, payload: Json): Promise<unknown | null> {
  try {
...
async function workStatusCommand(): Promise<void> {
  ...
  for (const worker of workers) {
    ...
    const modelText = modelSummaryText(isJson(worker.model) ? worker.model as Json : null);
...
function validateWorkerSessionSelector(sessionId: string): void {
  if (!/^[A-Za-z0-9._-]+$/.test(sessionId) || sessionId === '.' || sessionId === '..') {
    throw new OrbitCliError(`Invalid worker session id: ${sessionId}`);
  }
}

async function readLocalWorkerProcess(sessionId: string): Promise<LocalWorkerProcess | null> {
...
async function listLiveLocalWorkerProcesses(): Promise<LocalWorkerProcess[]> {
  let entries: string[] = [];
  try {
...
async function waitForWorkerNotAlive(worker: LocalWorkerProcess, timeoutMs: number): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() <= deadline) {
    const status = await workerProcessStatus(worker.sessionId, worker.pid, worker.state, worker.config);
    if (!status.alive || !status.verified) return true;
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  return false;
}

async function appendWorkerLogBestEffort(sessionId: string, message: string): Promise<void> {
  try {
...
async function markWorkerStoppedByUser(worker: LocalWorkerProcess, signal: string): Promise<void> {
...
async function stopLiveLocalWorker(worker: LocalWorkerProcess): Promise<Json> {
...
function printStopWorkResult(payload: Json): void {
  if (hasFlag('--json')) {
    console.log(JSON.stringify(payload, null, 2));
    return;
  }
...
function failStopWork(message: string, workers: LocalWorkerProcess[] = []): void {
  const payload = {
    ok: false,
    mode: 'stop-work',
    error: message,
...
async function stopWorkCommand(): Promise<void> {
  ...
  if (sessionId) validateWorkerSessionSelector(sessionId);
