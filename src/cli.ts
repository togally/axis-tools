...
function assertLevel1CapabilityDetailedDesign(
  ...
  const userOperationPanorama = projectKnowledgeSection(
    body,
    /^##\s+\d+\.?\s+用户业务操作全景\s*$/m,
  );
  if (!userOperationPanorama) {
    throw new Error(
      `project knowledge level-1 capability detailed design missing user operation panorama: ${capabilityId}`,
    );
  }
  ...
  if (headerLineIndex < 0 || panoramaLines.length <= headerLineIndex + 2) {
    throw new Error(
      `project knowledge level-1 capability detailed design missing user journey table: ${capabilityId}`,
    );
  }
  ...
  for (const requiredHeader of requiredHeaders) {
    if (!headers.includes(requiredHeader)) {
      throw new Error(
        `project knowledge level-1 capability user journey table missing ${requiredHeader}: ${capabilityId}`,
      );
    }
  }
  if (headers.length !== requiredHeaders.length
    ...
    throw new Error(
      `project knowledge level-1 capability user journey table does not match fixed schema: ${capabilityId}`,
    );
  }
  ...
  if (!separatorLine.startsWith('|')
    ...
    throw new Error(
      `project knowledge level-1 capability user journey table has malformed separator: ${capabilityId}`,
    );
  }
  const rows: string[][] = [];
  ...
  if (rows.some((cells) => cells.length !== headers.length)) {
    throw new Error(
      `project knowledge level-1 capability user journey table has malformed row: ${capabilityId}`,
    );
  }
  if (rows.length === 0) {
    throw new Error(
      `project knowledge level-1 capability detailed design has no user journeys: ${capabilityId}`,
    );
  }
  const field = (row: string[], name: string): string => row[headers.indexOf(name)] ?? '';
  ...
  const concreteData = (value: string, operation: '读取' | '写入'): boolean => {
    if (isGenericText(value)) return false;
  ...
  for (const row of rows) {
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9_.:\-]*$/.test(journeyId)) {
      throw new Error(
        `project knowledge level-1 capability user journey has invalid journey_id: ${capabilityId}/${journeyId}`,
      );
    }
    if (journeyIds.has(journeyId)) {
      throw new Error(
        `project knowledge level-1 capability user journey has duplicate journey_id: ${capabilityId}/${journeyId}`,
      );
    }
    ...
    if (!secondaryCapabilities.some((secondary) => secondary.secondary_capability_id === secondaryId)) {
      throw new Error(
        `project knowledge level-1 capability user journey references unknown secondary capability: ${capabilityId}/${journeyId}`,
      );
    }
    ...
    for (const requiredField of ['用户/角色', '提供的业务', '用户目标', '用户怎么操作', '用户可见结果']) {
      ...
      if (value.length < 2 || isGenericText(value)) {
        throw new Error(
          `project knowledge level-1 capability user journey missing concrete ${requiredField}: ${capabilityId}/${journeyId}`,
        );
      }
    }
    ...
    if (/现有入口集合|对应接口|对应入口/.test(interfaceEntry)) {
      throw new Error(
        `project knowledge level-1 capability user journey uses generic interface placeholder: ${capabilityId}/${journeyId}`,
      );
    }
    if (!/(?:\b(?:GET|POST|PUT|PATCH|DELETE)\s+\/[A-Za-z0-9_{}?&=./:\-]+|\b(?:EVENT|TOPIC|JOB|COMMAND)\s+[A-Za-z0-9_.:/\-]+)/.test(interfaceEntry)) {
      throw new Error(
        `project knowledge level-1 capability user journey missing concrete interface: ${capabilityId}/${journeyId}`,
      );
    }
    if (exactCodeAnchors(field(row, 'Controller/Handler')).length < 1) {
      throw new Error(
        `project knowledge level-1 capability user journey missing exact Controller/Handler anchor: ${capabilityId}/${journeyId}`,
      );
    }
    if (exactCodeAnchors(field(row, 'Service/UseCase')).length < 1) {
      throw new Error(
        `project knowledge level-1 capability user journey missing exact Service/UseCase anchor: ${capabilityId}/${journeyId}`,
      );
    }
    if (!concreteData(field(row, '读取数据'), '读取')) {
      throw new Error(
        `project knowledge level-1 capability user journey missing concrete read data: ${capabilityId}/${journeyId}`,
      );
    }
    if (!concreteData(field(row, '写入/产生数据'), '写入')) {
      throw new Error(
        `project knowledge level-1 capability user journey missing concrete written or produced data: ${capabilityId}/${journeyId}`,
      );
    }
    ...
    if (!new RegExp(`\\]\\((?:business/capabilities/${capabilityId}/)?${escapedChildPath}\\)`).test(
      ...
      throw new Error(
        `project knowledge level-1 capability user journey omits secondary capability link: ${capabilityId}/${journeyId}`,
      );
    }
    if (exactCodeAnchors(field(row, '证据')).length < 1) {
      throw new Error(
        `project knowledge level-1 capability user journey missing exact evidence anchor: ${capabilityId}/${journeyId}`,
      );
    }
  ...
  for (const secondary of secondaryCapabilities) {
    ...
    if (!journeySecondaryIds.has(secondaryId)) {
      throw new Error(
        `project knowledge level-1 capability user journeys omit secondary capability: ${capabilityId}/${secondaryId}`,
      );
    }
  }
    ...
    for (const secondary of capability.secondary_capabilities as SecondaryCapabilityInventoryItem[]) {
      ...
      for (const journeyId of childJourneyBindings.keys()) {
        if (!expectedJourneyIds.has(journeyId)) {
          throw new Error(
            `project knowledge secondary capability detailed design contains journey_id absent from level-1 panorama: ${capabilityId}/${secondaryId}/${journeyId}`,
          );
        }
      }
