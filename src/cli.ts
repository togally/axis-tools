...
function inlineYamlScalarItems(value: string): string[] {
  const items: string[] = [];
  let current = '';
  let quote: '"' | "'" | null = null;
  ...
  if (quote !== null) throw new Error(`Unsupported unterminated inline YAML string: ${value}`);
  items.push(current.trim());
  return items;
}

function parseScalar(value: string): string | boolean | SimpleYamlValue[] {
...
interface SecondaryCapabilityInventoryItem {
  ...
  name?: string;
  secondary_capability_name?: string;
  business_ids?: string[];
}
...
interface Level1CapabilityDependencyNode {
  level1_capability_id?: string;
  level1_capability_name?: string;
}

interface Level1CapabilityDependencyEdge {
  edge_id?: string;
  from_level1_capability_id?: string;
  to_level1_capability_id?: string;
  relation_type?: string;
  stage?: string;
  summary?: string;
  journey_ids?: string[] | string;
  api_ids?: string[] | string;
  evidence_refs?: string[] | string;
  confidence?: string;
}

interface Level1CapabilityDependencyGraph {
  schema?: string;
  schema_version?: string;
  derivation_status?: string;
  derivation_method?: string;
  derivation_revision?: string | number;
  gap_id?: string;
  nodes?: Level1CapabilityDependencyNode[] | string;
  edges?: Level1CapabilityDependencyEdge[] | string;
}

interface Level1CapabilityDependencyProjection {
  status: 'pending_level1_completion' | 'derived';
  revision: string;
  gapId: string;
  upstream: Set<string> | 'not_derived';
  downstream: Set<string> | 'not_derived';
}
...
interface Level1JourneySummary {
  journeyId: string;
  apiId: string;
  ...
  tableIds: string[];
  physicalTableNames: Map<string, string>;
}

interface SecondaryJourneyBinding {
  apiId: string;
  interfaceEntry: string;
  controllerAnchors: string[];
  serviceAnchors: string[];
...
function exactCodeAnchors(body: string): string[] {
  return [...body.matchAll(
    /(?<![A-Za-z0-9_./:@+\-])(?:[A-Za-z0-9_@+\-][A-Za-z0-9_.@+\-]*\/)+[A-Za-z0-9_$@+\-][A-Za-z0-9_.$@+\-]*\.[A-Za-z0-9]+:([1-9]\d*)-([1-9]\d*)#[A-Za-z_$][A-Za-z0-9_$<>.\-]*(?![A-Za-z0-9_$<>.()\/\-])/g,
  )]
...
function numberedMarkdownSubsections(body: string, chapterNumber: number): NumberedMarkdownSubsection[] {
...
function numberedMarkdownSubsubsections(
  body: string,
  chapterNumber: number,
  sectionNumber: number,
): NumberedMarkdownSubsection[] {
...
function verticalMarkdownTableFields(table: MarkdownTable): Map<string, string> | null {
  const headers = table.headers.map(normalizeMarkdownCell);
  if (headers.length !== 2 || headers[0] !== '项目' || headers[1] !== '内容') return null;
  return new Map(table.rows.map((row) => [normalizeMarkdownCell(row[0] ?? ''), row[1] ?? '']));
}

function exactVerticalMarkdownTableFields(
  table: MarkdownTable,
  expectedFields: string[],
  scope: string,
): Map<string, string> {
  const fields = verticalMarkdownTableFields(table);
  if (!fields) throw new Error(`project knowledge vertical table has invalid header: ${scope}`);
...
function markdownTableMatchesExactVerticalFields(
  table: MarkdownTable,
  expectedFields: string[],
): boolean {
  const fields = verticalMarkdownTableFields(table);
...
function mermaidDiagramBodies(body: string): string[] {
  return [...body.matchAll(/(`{3,}|~{3,})mermaid\s*([\s\S]*?)\1/gi)]
    .map((match) => match[2]);
}

function flowchartNodeLabelMap(diagram: string): Map<string, string> {
  const labels = new Map<string, string>();
  for (const nodeMatch of diagram.matchAll(/\b([A-Za-z][A-Za-z0-9_]*)\s*(?:\["([^"]+)"\]|\[([^\]]+)\]|\("([^"]+)"\)|\(([^)]+)\)|\{"([^"]+)"\}|\{([^}]+)\})/g)) {
    labels.set(nodeMatch[1], normalizeMarkdownCell(
      nodeMatch[2] ?? nodeMatch[3] ?? nodeMatch[4] ?? nodeMatch[5] ?? nodeMatch[6] ?? nodeMatch[7] ?? '',
    ));
  }
  return labels;
}

function flowchartEdgeEndpoints(diagram: string): {
  edgeCount: number;
  ...
  for (const rawLine of diagram.split(/\r?\n/)) {
    ...
    for (const statement of rawLine.split(';')) {
      const normalizedLine = statement
        ...
        .replace(/-->|==>/g, ' __AXIS_EDGE__ ');
      if (!normalizedLine.includes('__AXIS_EDGE__')) continue;
...
function assertAtomicInterfaceLogicDiagrams(body: string, scope: string): void {
...
function assertCompactPartialMethodDiagrams(body: string, scope: string): void {
  const diagrams = mermaidDiagramBodies(body)
    .filter((diagram) => /\b(?:flowchart|graph)\b/i.test(diagram));
  const exactMethodCallPattern = /^(?:[A-Za-z_$][A-Za-z0-9_$]*\.)+[A-Za-z_$][A-Za-z0-9_$]*\(\)$/;
...
function axisDocumentMetadata(body: string, scope: string): string {
...
function metadataScalar(metadata: string, key: string): string {
  const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`(?:^|\\s)${escapedKey}\\s*=\\s*([^\\s]+)`, 'm').exec(metadata)?.[1] ?? '';
}

function metadataIdentifierList(metadata: string, key: string): string[] {
  const raw = metadataScalar(metadata, key).replace(/^\[|\]$/g, '');
  return raw.split(/[,，、;；]+/).map((value) => value.trim()).filter(Boolean);
}

function visibleMarkdownBody(body: string): string {
  return body.replace(/<!--[\s\S]*?-->/g, '');
}

function markdownLocalLinkTargets(body: string): string[] {
...
function assertSecondaryNavigationLinks(
  body: string,
  documentPath: string,
  sourceRoot: string,
  expectedRelativeTargets: string[],
  scope: string,
): void {
...
function shortCodeAnchors(body: string): CompactCodeAnchor[] {
  return [...body.matchAll(
    /(?<![A-Za-z0-9_./:@+\-])([A-Za-z0-9_$@+\-][A-Za-z0-9_.$@+\-]*\.[A-Za-z0-9]+):([1-9]\d*)-([1-9]\d*)#([A-Za-z_$][A-Za-z0-9_$<>.\-]*)(?![A-Za-z0-9_$<>.()\/\-])/g,
  )]
    .filter((match) => Number(match[2]) <= Number(match[3]))
    .map((match) => compactCodeAnchor(match[0], match[1], match[2], match[3], match[4]));
}

function apparentVisibleCodeLocators(body: string): string[] {
  return [...visibleMarkdownBody(body).matchAll(
    /`([^`\n]*\.[A-Za-z0-9]+:[1-9]\d*-[1-9]\d*#[^`\n]+)`/g,
  )].map((match) => match[1].trim());
}

function apparentCodeLocators(body: string): string[] {
  return [...body.matchAll(
    /`([^`\n]*\.[A-Za-z0-9]+:[1-9]\d*-[1-9]\d*#[^`\n]+)`/g,
  )].map((match) => match[1].trim());
}

function assertCompactCodeLocatorAtomicity(body: string, scope: string): void {
...
function axisEvidenceAnchors(body: string): CompactCodeAnchor[] {
  return [...body.matchAll(/<!--\s*axis-evidence:\s*([\s\S]*?)\s*-->/gi)]
    ...
    .filter((anchor): anchor is CompactCodeAnchor => anchor !== null);
}

function hiddenExactCodeAnchors(body: string): CompactCodeAnchor[] {
...
function assertCompactEvidenceLocators(
  body: string,
  shortLocatorBodies: string[],
  scope: string,
): void {
  ...
  for (const locatorBody of shortLocatorBodies) {
    ...
    for (const anchor of anchors) {
      const exactMatches = new Set(
        evidenceAnchors.filter((evidence) => (
          evidence.fileName === anchor.fileName
...
function compactTopLevelSections(
  body: string,
  expected: Array<string | readonly string[]>,
  scope: string,
): Map<number, string> {
...
function axisNamedCommentBodies(body: string, name: string): string[] {
  const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return [...body.matchAll(new RegExp(`<!--\\s*${escapedName}\\b([\\s\\S]*?)-->`, 'gi'))]
    .map((match) => match[1]);
}

function assertCompactSingleLayerDiagrams(body: string, scope: string): void {
  const diagrams = mermaidDiagramBodies(body)
    .filter((diagram) => /\b(?:flowchart|graph)\b/i.test(diagram));
  const exactMethodCallPattern = /^(?:[A-Za-z_$][A-Za-z0-9_$]*\.)+[A-Za-z_$][A-Za-z0-9_$]*\(\)$/;
  if (diagrams.length === 0) {
...
function compactCanonicalContract(value: string): string {
  return normalizeMarkdownCell(value).replace(/\s+/g, ' ');
}

function compactDelimitedValues(value: string): string[] {
...
function compactContractMatchesType(contract: string, contractType: string): boolean {
  return contractType === 'HTTP'
    ? /^(?:GET|POST|PUT|PATCH|DELETE)\s+\//.test(contract)
    : contract.startsWith(`${contractType} `);
}

function compactStepIds(value: string): string[] {
  const steps: string[] = [];
  for (const match of normalizeMarkdownCell(value).matchAll(/\bS([1-9]\d*)(?:\s*[-–—~至]\s*S?([1-9]\d*))?\b/g)) {
    ...
    for (let index = start; index <= end; index += 1) steps.push(`S${index}`);
  }
  return steps;
}

function compactNextStepTargets(value: string): {
  valid: boolean;
  nextSteps: string[];
  ...
  for (const token of tokens) {
    if (/^(?:结束|终止|无后续|not_applicable)$/i.test(token)) {
      if (canTerminate) valid = false;
      canTerminate = true;
    } else if (/^S[1-9]\d*$/.test(token)) {
      if (nextSteps.includes(token)) valid = false;
...
function assertCompactBusinessDiagrams(body: string, scope: string): void {
  const diagrams = mermaidDiagramBodies(body)
    .filter((diagram) => /\b(?:flowchart|graph)\b/i.test(diagram));
  const exactMethodCallPattern = /^(?:[A-Za-z_$][A-Za-z0-9_$]*\.)+[A-Za-z_$][A-Za-z0-9_$]*\(\)$/;
  for (const diagram of diagrams) {
    ...
    if (nodeLabels.size < 2
      ...
      || !edges.traceable
      || [...edges.endpointIds].some((nodeId) => !nodeLabels.has(nodeId))
      || [...nodeLabels.values()].some((label) => (
        exactMethodCallPattern.test(label)
...
function visibleNumberedLevel2Headings(body: string): string[] {
  return [...visibleMarkdownBody(body).matchAll(/^##(?!#)\s+(\d+)\.?\s+(.+?)\s*$/gm)]
    .map((heading) => `${heading[1]}.${heading[2].trim()}`);
}

function isCompactPartialSecondaryCapabilityDetailedDesign(body: string): boolean {
  const visibleBody = visibleMarkdownBody(body);
...
function assertCompactPartialSecondaryCapabilityDetailedDesign(
  body: string,
  capabilityId: string,
  secondary: SecondaryCapabilityInventoryItem,
  gapReportBody: string,
  requiresCurrentReaderContract = false,
): void {
  ...
  if (currentCompactProfile) {
    ...
    const participantSteps = new Map<string, Set<string>>();
    ...
    const visibleFlowByStep = new Map<string, {
      participant: string;
      action: string;
      nextSteps: string[];
      canTerminate: boolean;
    }>();
    const actualStepsByParticipant = new Map<string, Set<string>>();
    for (let index = 0; index < visibleFlowTable.rows.length; index += 1) {
      ...
      const steps = actualStepsByParticipant.get(participant) ?? new Set<string>();
      steps.add(stepId);
    ...
    const incomingSteps = new Map<string, Set<string>>(
      [...visibleFlowByStep.keys()].map((stepId) => [stepId, new Set<string>()]),
    );
    ...
    const terminalSteps = [...visibleFlowByStep]
      ...
      .map(([stepId]) => stepId);
    const reachable = (roots: string[], reverse = false): Set<string> => {
    ...
    if (!interfaceNotApplicable && (machineAccessTables.length !== 1
      ...
      || machineAccessTables[0].rows.some((row) => (
        row.slice(0, 5).some((cell) => !normalizeMarkdownCell(cell))
    ...
    const accessApiIds = new Set<string>();
    const accessActorsByApi = new Map<string, Set<string>>();
    const accessContractByApi = new Map<string, string>();
    const accessApiByContract = new Map<string, string>();
    const accessActorApiPairs = new Set<string>();
    for (const row of machineAccessTables[0]?.rows ?? []) {
      ...
      const actors = accessActorsByApi.get(apiId) ?? new Set<string>();
    ...
    const flowApiIds = new Set<string>();
    const flowStepsByApi = new Map<string, Set<string>>();
    const tracedFlowSteps = new Set<string>();
    const flowMappingsByStep = new Map<string, Set<string>>();
    const flowParticipantsByApi = new Map<string, Set<string>>();
    const flowAuthorizedActorsByApi = new Map<string, Set<string>>();
    const flowRelationsByApi = new Map<string, Set<string>>();
    for (const row of flowMachineTables[0].rows) {
      ...
      const stepMappings = flowMappingsByStep.get(stepId) ?? new Set<string>();
      ...
      const mappedSteps = flowStepsByApi.get(apiId) ?? new Set<string>();
      mappedSteps.add(stepId);
      flowStepsByApi.set(apiId, mappedSteps);
      const participants = flowParticipantsByApi.get(apiId) ?? new Set<string>();
      participants.add(participant);
      flowParticipantsByApi.set(apiId, participants);
      const relations = flowRelationsByApi.get(apiId) ?? new Set<string>();
      relations.add(contractRelation);
      flowRelationsByApi.set(apiId, relations);
      if (/^(?:caller|producer)$/.test(contractRelation)) {
        const authorizedActors = flowAuthorizedActorsByApi.get(apiId) ?? new Set<string>();
    ...
    if (interfaceNotApplicable) {
      ...
      assertCompactEvidenceLocators(
        rawInterfaceSection,
        compactVisibleLocatorLines(rawInterfaceSection),
        `${scope}/5`,
      );
      assertCompactEvidenceLocators(
        [rawNumberedTopLevelSection(body, 1), rawNumberedTopLevelSection(body, 3)].join('\n'),
    ...
    if (visibleInterfaceGroups.length === 0
      || visibleInterfaceGroups.some((group, index) => (
        group.index !== index + 1
    ...
    if (interfaceGroups.length !== visibleInterfaceGroups.length
      || interfaceGroups.some((group, index) => (
        group.index !== visibleInterfaceGroups[index].index
    ...
    const interfaceSummaries = interfaceGroups.map((group, groupIndex) => {
      ...
      const fields = exactVerticalMarkdownTableFields(
        matchingTables[0],
        interfaceFields,
        `${scope}/5.${group.index}`,
      );
      ...
      const interfaceMachines = axisNamedCommentBodies(group.body, 'axis-interface-machine-table')
        ...
        .filter((machine): machine is Map<string, string> => machine !== null);
      ...
      assertCompactEvidenceLocators(
        group.body,
        compactVisibleLocatorLines(group.body),
        `${scope}/5.${group.index}`,
      );
      ...
      if (implementationTables.length !== 1
        || implementationTables.some((table) => {
          ...
          return headers.length !== 3
            ...
            || table.rows.length === 0
            || table.rows.some((row) => (
              row.some((cell) => !normalizeMarkdownCell(cell))
      ...
      if (visibleImplementationLocators.length === 0
        || visibleImplementationLocators.some((locator) => (
          implementationAnchors.filter((anchor) => (
            anchor.fileName === locator.fileName
            && anchor.lineStart === locator.lineStart
            && anchor.lineEnd === locator.lineEnd
            && anchor.symbol === locator.symbol
          )).length !== 1
        ))) {
    ...
    const interfaceApiIds = new Set<string>();
    const interfaceApiByContract = new Map<string, string>();
    for (let index = 0; index < interfaceSummaries.length; index += 1) {
      ...
      const allowedRelations = contractType === 'HTTP'
        ...
        : /^(?:EVENT|TOPIC)$/.test(contractType)
          ...
          : new Set(['caller', 'handler']);
      const contractRelations = flowRelationsByApi.get(apiId) ?? new Set<string>();
      if (!projectKnowledgeTraceIdentifierPattern.test(apiId)
        || invalidProjectKnowledgeTraceIdentifierPattern.test(apiId)
...
function isCompactPartialLevel1CapabilityDetailedDesign(body: string): boolean {
  const visibleBody = visibleMarkdownBody(body);
  const headings = visibleNumberedLevel2Headings(body);
  return declaresCompactReaderProfile(body)
    || (headings.length === 6
...
function assertCompactPartialLevel1CapabilityDetailedDesign(
  body: string,
  capabilityId: string,
  secondaryCapabilities: SecondaryCapabilityInventoryItem[],
  gapReportBody: string,
): Map<string, Level1JourneySummary[]> {
  const sections = compactTopLevelSections(body, [
    ['设计结论与能力边界', '能力边界'],
    ['二级能力完整性与导航', '二级能力'],
    ['对外业务能力与接口实现', '对外业务入口'],
  ...
  if (currentCompactProfile) {
    const journeyTables = axisNamedCommentBodies(body, 'axis-journey-machine-table')
      .flatMap((commentBody) => markdownTables(commentBody))
      .map((table) => verticalMarkdownTableFields(table))
      .filter((fields): fields is Map<string, string> => fields !== null);
    const journeySecondaryIds = journeyTables.map((fields) => (
      normalizeMarkdownCell(fields.get('secondary_capability_id') ?? '')
    ));
    if (journeyTables.length < secondaryCapabilities.length
      || journeyTables.some((fields) => (
        ['level1_journey_id', 'flow_id', 'secondary_capability_id', 'api_id', '代表入口']
    ...
    for (const secondary of secondaryCapabilities) {
      ...
      if (journeySecondaryIds.filter((value) => value === secondaryId).length !== 1) {
        throw new Error(
          `project knowledge compact partial level-1 business entry omits secondary capability: ${capabilityId}/${secondaryId}`,
        );
      }
    }
    ...
    return new Map(secondaryCapabilities.map((secondary) => [
      secondary.secondary_capability_id as string,
      [],
    ]));
  }
...
function assertSecondaryAccessMatrix(
  body: string,
  scope: string,
  journeyBindings: Map<string, SecondaryJourneyBinding[]>,
): void {
  ...
  const interfaceBindingsByApi = new Map(
    [...journeyBindings.values()].flat().map((binding) => [binding.apiId, binding]),
  );
...
function secondaryInterfaceTraceBindings(
  ...
  chapterNumber: number,
  scope: string,
  enforceAtomicMethodDiagrams = false,
  ...
  for (let groupIndex = 0; groupIndex < groupMatches.length; groupIndex += 1) {
    ...
    const accessControlDimensions = new Map<string, string[]>();
    ...
    const securityAcceptanceDimensions = new Map<string, string[]>();
...
function traceIdentifierList(value: string): string[] {
  return normalizeMarkdownCell(value).split(/[,，、;；\s]+/).filter(Boolean);
}

function assertLevel1BusinessSemantics(
  body: string,
  capabilityId: string,
  secondaryCapabilities: SecondaryCapabilityInventoryItem[],
): void {
  ...
  const knownSecondaryIds = new Set(
    secondaryCapabilities.map((secondary) => secondary.secondary_capability_id as string),
  );
...
function assertLevel1CapabilityTableDesign(
  body: string,
  capabilityId: string,
  secondaryCapabilities: SecondaryCapabilityInventoryItem[],
  knownApiIds: Set<string>,
  stepTableIds: Set<string>,
  apiSecondaryIds: Map<string, Set<string>>,
  stepTableApiIds: Map<string, Set<string>>,
  stepTableSecondaryIds: Map<string, Set<string>>,
  gapReportBody: string,
): Map<string, string> {
  ...
  const controlLines = tableDesignSection.split(/\r?\n/).filter((line) => (
    /\btable_design_status\s*=/.test(line)
  ...
  if (status === 'not_applicable') {
    ...
    const reasonFields = reasonTables.length === 1
      ? exactVerticalMarkdownTableFields(
        reasonTables[0],
        ['table_design_status', '原因', '证据'],
        `${capabilityId}/table-design-not-applicable`,
      )
      : null;
    if (!reasonFields
  ...
  const physicalTableNames = new Map<string, string>();
  const tableApiIds = new Map<string, Set<string>>();
  for (const row of inventoryTable.rows) {
    ...
    if (!sameIdentifierSet(secondaryIdSet, stepTableSecondaryIds.get(tableId) ?? new Set<string>())
      || !sameIdentifierSet(new Set(apiIds), stepTableApiIds.get(tableId) ?? new Set<string>())) {
    ...
    if (apiIds.some((apiId) => {
      const owners = apiSecondaryIds.get(apiId) ?? new Set<string>();
  ...
  const erEntityNames = new Set<string>();
  ...
  for (const rawLine of erLines) {
    ...
    if (!line || line.startsWith('%%') || /^erDiagram\b/.test(line)) continue;
    ...
    if (entityMatch) erEntityNames.add(entityMatch[1] ?? entityMatch[2]);
  }
  for (const tableId of tableIds) {
    const physicalTableName = physicalTableNames.get(tableId) as string;
  ...
  const fieldTableIds = new Set<string>();
  ...
  for (const [fieldSubsectionIndex, subsection] of fieldSubsections.entries()) {
    ...
    if (tableIdMatches.length === 0) continue;
  ...
  if (tableIds.size > 1) {
    ...
    for (const row of relationshipTable.rows) {
      ...
      if (cardinality !== '无直接关系') {
        ...
        const sourcePhysicalName = physicalTableNames.get(sourceTableId) as string;
        const targetPhysicalName = physicalTableNames.get(targetTableId) as string;
        const evidencedRelationshipIndex = erRelationships.findIndex((relationship, relationshipIndex) => {
          if (matchedErRelationshipIndexes.has(relationshipIndex)) return false;
        ...
        const sourcePhysicalName = physicalTableNames.get(sourceTableId) as string;
        const targetPhysicalName = physicalTableNames.get(targetTableId) as string;
        if (erRelationships.some((relationship) => (
          (relationship.source === sourcePhysicalName && relationship.target === targetPhysicalName)
...
function assertLevel1CapabilityDetailedDesign(
  body: string,
  capabilityId: string,
  secondaryCapabilities: SecondaryCapabilityInventoryItem[],
  gapReportBody: string,
  ...
  if (/^##\s+\d+\.?\s+(?:用户业务操作全景|跨二级能力用户旅程|跨模块协作|共享业务语义与一级治理)\s*$/m.test(body)) {
    throw new Error(
      `project knowledge level-1 capability detailed design uses a legacy flat or separate cross-capability section: ${capabilityId}`,
    );
  }
  const outwardCapabilitiesSection = projectKnowledgeSection(
    body,
    /^##\s+3\.?\s+对外业务能力与接口实现\s*$/m,
  );
  if (!outwardCapabilitiesSection) {
    throw new Error(
      `project knowledge level-1 capability detailed design missing outward capability implementation: ${capabilityId}`,
    );
  }
  ...
  const journeyControlLines = body.split(/\r?\n/).filter((line) => (
    /\buser_journey_design_status\s*=/.test(line)
    && /\buser_journey_coverage\s*=/.test(line)
    && /\buser_journey_gap_id\s*=/.test(line)
  ));
  if (journeyControlLines.length !== 1) {
    throw new Error(
      `project knowledge level-1 capability detailed design requires one user journey control line: ${capabilityId}`,
    );
  }
  const journeyControlLine = journeyControlLines[0];
  if (!/\buser_journey_design_status\s*=\s*detailed\b/.test(journeyControlLine)) {
    throw new Error(
      `project knowledge level-1 capability detailed design missing user_journey_design_status: ${capabilityId}`,
    );
  }
  if (!/\buser_journey_coverage\s*=\s*(?:complete|partial)\b/.test(journeyControlLine)) {
    throw new Error(
      `project knowledge level-1 capability detailed design missing user_journey_coverage: ${capabilityId}`,
    );
  }
  ...
  if (journeyCoverage === 'partial' && (!journeyGapId || journeyGapId === 'not_applicable')) {
    throw new Error(
      `project knowledge level-1 capability partial user journey coverage requires an explicit gap: ${capabilityId}`,
    );
  }
  if (journeyCoverage === 'complete' && journeyGapId !== 'not_applicable') {
    throw new Error(
      `project knowledge level-1 capability complete user journey coverage requires user_journey_gap_id=not_applicable: ${capabilityId}`,
    );
  }
  if (journeyCoverage === 'partial' && journeyGapId) {
    const gapSection = projectKnowledgeSection(
      body,
      /^##\s+\d+\.?\s+(?:缺口与覆盖说明|验收、证据与缺口)\s*$/m,
    );
    ...
    if (!gapSection
      ...
      throw new Error(
        `project knowledge level-1 capability partial user journey gap is not traced in overview and gap report: ${capabilityId}/${journeyGapId}`,
      );
    }
  }
  ...
  const journeySecondaryIds = new Set<string>();
  const journeyIds = new Set<string>();
  const stepIds = new Set<string>();
  const apiIds = new Set<string>();
  const stepTableIds = new Set<string>();
  const apiSecondaryIds = new Map<string, Set<string>>();
  const stepTableApiIds = new Map<string, Set<string>>();
  const stepTableSecondaryIds = new Map<string, Set<string>>();
  const journeyIdsBySecondary = new Map<string, Level1JourneySummary[]>();
  ...
  const concreteData = (value: string, operation: '读取' | '写入'): boolean => {
    const normalized = normalizeMarkdownCell(value);
    if (isGenericText(value)
      || /\{[^}]+\}|TODO|TBD|待补|待定/i.test(normalized)) return false;
    ...
    if (concreteCodeToken) return true;
  ...
  for (const outwardCapability of outwardCapabilities) {
    ...
    const subsubsections = numberedMarkdownSubsubsections(
      outwardCapability.body,
      3,
      outwardCapability.index,
    );
    ...
    if (subsubsections.length !== expectedSubsubsectionTitles.length
      ...
      || subsubsections.some((subsection, index) => (
    ...
    const summary = exactVerticalMarkdownTableFields(
      summaryTables[0],
      summaryFields,
    ...
    const implementationSteps = implementationStepTables.map((table, index) => (
      exactVerticalMarkdownTableFields(
        table,
        implementationStepFields,
    ...
    for (const nodeMatch of diagram.matchAll(/\b([A-Za-z][A-Za-z0-9_]*)\s*(?:\["([^"]+)"\]|\[([^\]]+)\]|\("([^"]+)"\)|\(([^)]+)\))/g)) {
      ...
      const label = normalizeMarkdownCell(
        nodeMatch[2] ?? nodeMatch[3] ?? nodeMatch[4] ?? nodeMatch[5] ?? '',
      );
    ...
    const nodeMatchesIdentifier = (nodeId: string, label: string, identifier: string): boolean => {
    ...
    const edgeLikeLineCount = diagram.split(/\r?\n/).filter((line) => (
      !line.trim().startsWith('%%') && /(?:-->|==>|-\.->)/.test(line)
    )).length;
    ...
    const diagramEdges = allDiagramEdges.filter(
      (edge): edge is typeof edge & { label: string } => edge.label !== null,
    );
    const stepSecondaryIds = new Set<string>();
    let previousSecondaryId: string | null = null;
    let previousTargetNodeId: string | null = null;
    let entryNodeId: string | null = null;
    let previousEdgeLineIndex = -1;
    const implementationEdgeLineIndexes = new Set<number>();
    for (const step of implementationSteps) {
      ...
      if (!new RegExp(`\\]\\((?:business/capabilities/${capabilityId}/)?${escapedChildPath}\\)`).test(
        step.get('二级能力详情') ?? '',
      )) {
      ...
      if ([...nodeLabels.values()].some((label) => (
        label.includes(apiId) || label.includes(normalizedInterfaceEntry)
      ))) {
      ...
      const matchingEdge = diagramEdges.find((edge) => (
        !implementationEdgeLineIndexes.has(edge.lineIndex)
      ...
      const legacyJourneyEntryNode = sourceLabel.includes(
        normalizeMarkdownCell(summary.get('用户/角色') ?? ''),
      ) && sourceLabel.includes(
        normalizeMarkdownCell(summary.get('用户怎么操作') ?? ''),
      );
      ...
      if (entryNodeId === null) entryNodeId = matchingEdge.sourceNodeId;
      previousSecondaryId = secondaryId;
      ...
      journeyIdsBySecondary.set(secondaryId, [
        ...(journeyIdsBySecondary.get(secondaryId) ?? []),
        {
          ...
          physicalTableNames: new Map<string, string>(),
        },
      ]);
    }
    ...
    const visibleResultEdge = previousTargetNodeId === null
      ? undefined
      : allDiagramEdges.find((edge) => (
        edge.sourceNodeId === previousTargetNodeId
        && edge.lineIndex > previousEdgeLineIndex
    ...
    while (pendingNodeIds.length > 0) {
      ...
      for (const neighbor of adjacency.get(nodeId) ?? []) {
        if (!visitedNodeIds.has(neighbor)) pendingNodeIds.push(neighbor);
      }
    }
  ...
  const physicalTableNames = assertLevel1CapabilityTableDesign(
    body,
    capabilityId,
    ...
    apiSecondaryIds,
    stepTableApiIds,
    stepTableSecondaryIds,
    gapReportBody,
  );
  for (const journeys of journeyIdsBySecondary.values()) {
    for (const journey of journeys) {
      journey.physicalTableNames = new Map(
        journey.tableIds.map((tableId) => [tableId, physicalTableNames.get(tableId) as string]),
      );
    }
  }
...
function level1DependencyProjectionIds(
  rawValue: string,
  capabilityId: string,
  direction: 'upstream' | 'downstream',
): Set<string> | 'not_derived' {
  const value = normalizeMarkdownCell(rawValue);
  if (value === 'not_derived') return 'not_derived';
  ...
  if (ids.length === 0
    ...
    throw new Error(
      `project knowledge level-1 capability has invalid ${direction} dependency projection: ${capabilityId}`,
    );
  }
  return new Set(ids);
}

function level1CapabilityDependencyProjection(
  body: string,
  capabilityId: string,
  compactPartial = false,
): Level1CapabilityDependencyProjection {
  ...
  if (compactPartial) {
    if (control.status !== 'pending_level1_completion'
      ...
      throw new Error(
        `project knowledge compact partial level-1 dependency projection is invalid: ${capabilityId}`,
      );
    }
    return {
      ...control,
  ...
  if (upstreamRows.length !== 1 || downstreamRows.length !== 1) {
    throw new Error(
      `project knowledge level-1 capability requires one upstream and downstream dependency projection: ${capabilityId}`,
    );
  }
  ...
  for (const row of [upstreamRows[0], downstreamRows[0]]) {
    if (normalizeMarkdownCell(row[2] ?? '') !== expectedSource) {
      throw new Error(
        `project knowledge level-1 capability dependency projection has wrong graph source: ${capabilityId}`,
      );
    }
  }
  return {
...
function dependencyGraphControl(
  body: string,
  scope: string,
): Pick<Level1CapabilityDependencyProjection, 'status' | 'revision' | 'gapId'> {
  const controlLines = body.split(/\r?\n/).filter((line) => (
    /\bdependency_graph_status\s*=/.test(line)
    && /\bdependency_graph_revision\s*=/.test(line)
    && /\bdependency_graph_gap_id\s*=/.test(line)
  ));
  if (controlLines.length !== 1) {
    throw new Error(
      `project knowledge ${scope} requires one dependency graph projection control line`,
    );
  }
  const controlLine = controlLines[0];
  const status = /\bdependency_graph_status\s*=\s*(pending_level1_completion|derived)\b/
    .exec(controlLine)?.[1] as Level1CapabilityDependencyProjection['status'] | undefined;
  ...
  if (!status || !revision || !gapId) {
    throw new Error(
      `project knowledge ${scope} has invalid dependency graph projection control line`,
    );
  }
  return {
    status,
...
function sameIdentifierSet(left: Set<string>, right: Set<string>): boolean {
  return left.size === right.size && [...left].every((value) => right.has(value));
}

function graphArray<T>(value: T[] | string | undefined, field: string): T[] {
  if (Array.isArray(value)) return value;
  if (value === '[]') return [];
  throw new Error(`project knowledge level-1 capability dependency graph has invalid ${field}`);
}

function optionalGraphArray<T>(value: T[] | string | undefined, field: string): T[] {
  if (value === undefined) return [];
  return graphArray(value, field);
}

function assertLevel1CapabilityDependencyGraph(
  graphBody: string,
  capabilities: Level1CapabilityInventoryItem[],
  coverageByCapability: Map<string, 'complete' | 'partial'>,
  journeysByCapability: Map<string, Map<string, Level1JourneySummary[]>>,
  apiIdsByCapability: Map<string, Set<string>>,
  allSecondaryInterfacesComplete: boolean,
  projectionsByCapability: Map<string, Level1CapabilityDependencyProjection>,
  businessArchitectureBody: string,
  sourceRoot: string,
  gapReportBody: string,
): void {
  ...
  try {
    ...
    throw new Error(
      `project knowledge level-1 capability dependency graph is invalid YAML: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  const graph = parsedGraph as Level1CapabilityDependencyGraph;
  ...
  const expectedNames = new Map(capabilities.map((capability) => [
    capability.level1_capability_id as string,
    capability.level1_capability_name as string,
  ]));
  ...
  const assertBusinessArchitectureState = (): void => {
  ...
  if (!allComplete) {
    ...
    for (const [capabilityId, projection] of projectionsByCapability) {
      if (projection.status !== 'pending_level1_completion'
        ...
        throw new Error(
          `project knowledge level-1 capability must keep dependency projection not_derived until global analysis: ${capabilityId}`,
        );
      }
    }
    return;
  ...
  const incoming = new Map<string, Set<string>>(nodeIds.map((nodeId) => [nodeId, new Set<string>()]));
  const outgoing = new Map<string, Set<string>>(nodeIds.map((nodeId) => [nodeId, new Set<string>()]));
  const edgeIds = new Set<string>();
  const edgeKeys = new Set<string>();
  const journeyIdsByCapability = new Map<string, Set<string>>(
    [...journeysByCapability].map(([capabilityId, journeysBySecondary]) => [
      capabilityId,
  ...
  for (const edge of edges) {
    ...
    for (const journeyId of journeyIds) {
      if (typeof journeyId !== 'string'
        ...
        throw new Error(
          `project knowledge level-1 capability dependency graph edge references unknown journey_id: ${edgeId}/${String(journeyId)}`,
        );
      }
    }
    for (const apiId of apiIds) {
      if (typeof apiId !== 'string'
        ...
        throw new Error(
          `project knowledge level-1 capability dependency graph edge references unknown api_id: ${edgeId}/${String(apiId)}`,
        );
      }
    }
    const hasTraceableEvidence = evidenceRefs.every((ref) => {
      ...
      if (exactCodeAnchors(ref).length > 0) return true;
  ...
  for (const [capabilityId, projection] of projectionsByCapability) {
    ...
    if (projection.upstream === 'not_derived'
      || !sameIdentifierSet(projection.upstream, incoming.get(capabilityId) ?? new Set<string>())) {
    ...
    if (projection.downstream === 'not_derived'
      || !sameIdentifierSet(projection.downstream, outgoing.get(capabilityId) ?? new Set<string>())) {
...
function assertSecondaryCapabilityDetailedDesign(
  ...
  for (const sectionTitle of legacyTopLevelTitles) {
    ...
    if (new RegExp(`^##\\s+\\d+\\.?\\s+${escapedTitle}\\s*$`, 'm').test(body)) {
      throw new Error(
        `project knowledge secondary capability detailed design uses legacy top-level interface-local section: ${scope}/${sectionTitle}`,
      );
    }
  }
  ...
  if (interfaceStatus === 'not_applicable') {
    ...
    const interfaceBindings = secondaryInterfaceTraceBindings(
      interfaceSection,
      Number(interfaceHeading[1]),
      scope,
      /\breader_profile\s*=\s*strict_full\b/.test(body),
    );
...
async function projectKnowledgeSourceFiles(sourceRoot: string): Promise<ProjectKnowledgeSourceFile[]> {
  ...
  const level1JourneysByCapability = new Map<string, Map<string, Level1JourneySummary[]>>();
  const level1JourneyCoverageByCapability = new Map<string, 'complete' | 'partial'>();
  const level1DependencyProjectionsByCapability = new Map<string, Level1CapabilityDependencyProjection>();
  const compactPartialLevel1CapabilityIds = new Set<string>();
  const participantReaderContractLevel1CapabilityIds = new Set<string>();
  for (const capability of level1Capabilities) {
    const capabilityId = capability.level1_capability_id as string;
    ...
    const capabilityCoverage = /\buser_journey_coverage\s*=\s*(complete|partial)\b/
      .exec(capabilityDocumentBody)?.[1] as 'complete' | 'partial';
    ...
    level1JourneysByCapability.set(
      capabilityId,
      compactPartial
        ? assertCompactPartialLevel1CapabilityDetailedDesign(
          capabilityDocumentBody,
          capabilityId,
          capability.secondary_capabilities as SecondaryCapabilityInventoryItem[],
          gapReportBody,
        )
        : assertLevel1CapabilityDetailedDesign(
          capabilityDocumentBody,
          capabilityId,
          capability.secondary_capabilities as SecondaryCapabilityInventoryItem[],
          gapReportBody,
    ...
    level1DependencyProjectionsByCapability.set(
      capabilityId,
      level1CapabilityDependencyProjection(capabilityDocumentBody, capabilityId, compactPartial),
    );
  ...
  const apiIdsByCapability = new Map<string, Set<string>>(
    level1CapabilityIds.map((capabilityId) => [capabilityId, new Set<string>()]),
  );
  let allSecondaryInterfacesComplete = true;
  for (const capability of level1Capabilities) {
    const capabilityId = capability.level1_capability_id as string;
    const secondaries = capability.secondary_capabilities as SecondaryCapabilityInventoryItem[];
    for (let secondaryIndex = 0; secondaryIndex < secondaries.length; secondaryIndex += 1) {
      ...
      if (compactPartial && usesParticipantFlowInterfaceContract(body)) {
        ...
        if (secondaryIndex > 0) {
          expectedNavigationTargets.push(
            `business/capabilities/${capabilityId}/secondary-capabilities/${secondaries[secondaryIndex - 1].secondary_capability_id}/detailed-design.md`,
          );
        }
        ...
        assertSecondaryNavigationLinks(
          body,
          absolutePath,
          sourceRoot,
          expectedNavigationTargets,
          `${capabilityId}/${secondaryId}`,
        );
      }
      if (compactPartial !== compactPartialLevel1CapabilityIds.has(capabilityId)) {
        throw new Error(
          `project knowledge compact partial document profile mismatch: ${capabilityId}/${secondaryId}`,
        );
      }
      if (compactPartial) {
        assertCompactPartialSecondaryCapabilityDetailedDesign(
          body,
          capabilityId,
          secondary,
          gapReportBody,
          participantReaderContractLevel1CapabilityIds.has(capabilityId),
        );
      } else {
      ...
      const secondaryInterfaceCoverage = /\binterface_coverage\s*=\s*(complete|partial|not_applicable)\b/
        .exec(body)?.[1] ?? 'missing';
      if (secondaryInterfaceCoverage !== 'complete') allSecondaryInterfacesComplete = false;
      if (level1JourneyCoverageByCapability.get(capabilityId) === 'complete'
        && secondaryInterfaceCoverage !== 'complete') {
        throw new Error(
          `project knowledge level-1 complete user journey coverage conflicts with ${secondaryInterfaceCoverage} secondary interface coverage: ${capabilityId}/${secondaryId}`,
        );
      }
      if (compactPartial) {
      ...
      for (const bindings of childJourneyBindings.values()) {
        for (const binding of bindings) apiIdsByCapability.get(capabilityId)?.add(binding.apiId);
      }
      for (const journey of expectedJourneys) {
        const bindings = childJourneyBindings.get(journey.journeyId) ?? [];
        if (bindings.length === 0) {
          throw new Error(
            `project knowledge secondary capability detailed design omits level-1 journey_id: ${capabilityId}/${secondaryId}/${journey.journeyId}`,
          );
        }
        const matchingBinding = bindings.some((binding) => (
          binding.apiId === journey.apiId
          ...
          && journey.tableIds.every((tableId) => (
            binding.physicalTableNames.get(tableId) === journey.physicalTableNames.get(tableId)
          ))
        ));
        if (!matchingBinding) {
          throw new Error(
            `project knowledge secondary capability detailed design mismatches level-1 journey trace: ${capabilityId}/${secondaryId}/${journey.journeyId}`,
          );
        }
      }
      for (const [journeyId, bindings] of childJourneyBindings) {
        if (!expectedJourneyIds.has(journeyId)) {
          throw new Error(
            `project knowledge secondary capability detailed design contains journey_id absent from level-1 outward capabilities: ${capabilityId}/${secondaryId}/${journeyId}`,
          );
        }
        const expectedBindings = expectedJourneys.filter((journey) => journey.journeyId === journeyId);
        for (const binding of bindings) {
          const matchesParentBinding = expectedBindings.some((journey) => (
            binding.apiId === journey.apiId
            && binding.interfaceEntry === journey.interfaceEntry
            && journey.controllerAnchors.every((anchor) => binding.controllerAnchors.includes(anchor))
            && journey.serviceAnchors.every((anchor) => binding.serviceAnchors.includes(anchor))
          ...
          if (!matchesParentBinding) {
            throw new Error(
              `project knowledge secondary capability detailed design contains interface binding absent from level-1 outward capability: ${capabilityId}/${secondaryId}/${journeyId}/${binding.apiId}`,
            );
          }
        }
      }
  ...
  if (!existsSync(capabilityDependencyGraphPath)) {
    throw new Error(
      `project knowledge level-1 capability dependency graph missing: ${capabilityDependencyGraphRelativePath}`,
    );
  }
  assertLevel1CapabilityDependencyGraph(
    await readFile(capabilityDependencyGraphPath, 'utf8'),
    level1Capabilities,
    level1JourneyCoverageByCapability,
    level1JourneysByCapability,
    apiIdsByCapability,
    allSecondaryInterfacesComplete,
    level1DependencyProjectionsByCapability,
    businessArchitectureBody,
    sourceRoot,
    gapReportBody,
  );
