...
export interface ApplyTransactionOptions {
  ...
  validateBefore?: () => Promise<void>;
}

interface JournalEntry {
  role: string;
  path: string;
  original: { state: 'present' | 'absent'; sha256: string | null; backup: string | null };
  next: { state: 'present' | 'absent'; sha256: string | null };
  temp: string | null;
}
...
interface Journal {
  ...
  state: 'prepared' | 'replacing' | 'committed';
...
const defaultHooks: Required<TransactionHooks> = {
  copyFile,
...
function absoluteRepoPath(repo: string, relativePath: string): string {
  ...
  if (!absolute.startsWith(prefix)) throw new Error(`transaction path must be inside repo: ${relativePath}`);
  return absolute;
}
...
async function syncDirectoryFor(filePath: string, hooks: Required<TransactionHooks>): Promise<void> {
...
async function persistJournal(
  journalPath: string,
  journal: Journal,
  hooks: Required<TransactionHooks>,
): Promise<void> {
...
): { entry: JournalEntry; absolutePath: string; backupPath: string; tempPath: string } {
  ...
  return {
    ...
    entry: {
      ...
      original: {
        ...
        sha256: originalPresent ? hashText(file.originalText as string) : null,
        backup: originalPresent ? path.relative(repo, backupPath).split(path.sep).join('/') : null,
      },
      next: {
        state: nextPresent ? 'present' : 'absent',
        sha256: nextPresent ? hashText(file.nextText as string) : null,
      },
...
async function restoreOriginal(repo: string, journal: Journal, hooks: Required<TransactionHooks>): Promise<void> {
...
async function verifyNext(repo: string, journal: Journal): Promise<void> {
  for (const entry of journal.files) {
    const text = await readOptional(absoluteRepoPath(repo, entry.path));
    if (entry.next.state === 'absent') {
      if (text !== null) throw new Error(`committed transaction target is not absent: ${entry.path}`);
      continue;
    }
...
async function cleanup(repo: string, journalPath: string, journal: Journal, hooks: Required<TransactionHooks>): Promise<void> {
...
export async function applyTransaction(options: ApplyTransactionOptions): Promise<void> {
  ...
  const paths: Array<{ file: TransactionFile; absolutePath: string; backupPath: string; tempPath: string }> = [];
  ...
  for (const item of paths) {
    ...
    if (current !== item.file.originalText) throw new Error(`stale project-init target: ${item.file.path}`);
  }
...
export async function recoverTransaction(repoInput: string): Promise<void> {
