---
name: axis-doc-project-knowledge-bootstrap
...
2. Execute generation.
   Scan repository evidence, write both global architecture documents, build inventory, then compose `$axis-doc-business-domain` once for each unique `business_id`. Record unsupported claims as assumptions or gaps.
3. Verify the result.
...
public_safety:
  ...
  validation:
    ...
    findings_count: 0
generated_fields:
  generated_by_skill: axis-doc-project-knowledge-bootstrap
  source_commit: null
```
...
5. Generate every `business/domains/{business_id}/detailed-design.md` through `$axis-doc-business-domain`.
...
Report generated paths, inventory domain count, evidence coverage, confidence boundaries, gap summary, validation commands, snapshot run ID, and OSS sync status. For a single confirmed function, use `axis-doc-feature-detailed-design`; it must stop and ask the user when feature resolution has zero or multiple matches.
