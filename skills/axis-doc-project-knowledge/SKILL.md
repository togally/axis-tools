...
The frozen detailed-design rendering prompt is [secondary-capability-detailed-design-prompt-evidence-gated.md](references/secondary-capability-detailed-design-prompt-evidence-gated.md). It first returns `render_status=ready|blocked`; blocked evidence must be repaired before any reader document is rendered. Its exact model tiers, inference settings, artifact hashes, diagnostic matrix, frozen holdout and deterministic ranking are recorded in [secondary-capability-detailed-design-prompt-selection.json](references/secondary-capability-detailed-design-prompt-selection.json); normal runs must not silently substitute an earlier candidate or edit an artifact without rerunning selection.
...
Then run a separate project-wide reader evidence gate. Boundary evidence proves only that a secondary capability exists; inventory `evidence_refs`, a representative method, or one boundary anchor never proves complete participants, flow or interfaces. For every locked secondary capability, assemble a reader evidence card containing concrete business participants and responsibilities, atomic actions and state/branch results, exact externally callable or asynchronously observable contracts, permission/data scope, and evidence bindings into the implementation chain. Execute the frozen evidence-gated prompt against every card. Continue rendering only for `render_status=ready`; for `blocked`, preserve the returned gap codes, expand the search and do not fill missing facts with generic text. The project-wide reader gate passes only when every locked secondary is ready. One polished level-1 capability, marker presence, or structurally valid child set can never stand in for complete project coverage.
...
- never write reusable scaffolding such as “发起有证据支持的契约”“复核调用权威”“形成并返回已锁定的业务结果” as reader facts, and never convert an internal class, method, scheduler class or schedule label into `COMMAND internal:Class.method`, `JOB internal:Class.method`, `COMMAND Class.method`, `JOB XxxJob.method` or `JOB XxxTask schedule`; these are implementation evidence, not contracts;
...
- Every locked secondary passes the separate reader evidence gate; no child contains generic participant/flow scaffolding or an interface derived from an internal method.
