...
`detailed` interface design requires a concrete HTTP path, event/topic, job or command; field-level request and response contracts; error mapping; and exact entry, service, persistence and test anchors. Section 5 is grouped by contract rather than by field type: every direct `### 5.N` interface/event/job/command owns `#### 5.N.1 接口清单与代码追溯`, `5.N.2 请求字段`, `5.N.3 响应字段`, `5.N.4 错误码与异常映射`, and `5.N.5 认证、授权、幂等与事务`. The subsection prefix must match its parent, so the second contract uses `5.2.1` through `5.2.5`. Do not flatten multiple contracts into one wide table or global request/response sections. `partial` coverage requires a stable gap identifier. `not_applicable` requires a reason and an exact repository evidence anchor; a fieldless or one-way contract uses an explicit evidence-backed `not_applicable` row instead of an empty subsection.
