...
For a publish run, use at least one current in-App signal and one current public source. After current App evidence verifies operated categories and qualifications, inspect `商机中心` tabs `追抖音热词` and `跟潜力爆品`; `商品管理` area `发布潜力商品`; and `源头好货` sections `抖音爆款榜` and `热搜商机` with `流量扶持` when shown. For each named route, record whether every user-required opportunity metric is displayed in its card or representative detail schema. Exhaust only metric-capable routes. When a required metric is structurally absent, record `ineligible_by_metric_unavailable` with the missing field and result/page count; when an invoked entry has no separate result surface, record `unavailable_in_current_app`. Neither terminal produces an offer or listing. Route A/B products from metric-capable routes obtain offers through `找货源`; Route C metric-capable cards already represent source-backed candidates. Prefer official platform rules and first-party/public market sources. Third-party pages are comparison samples, not market-wide truth.
...
For every included in-stock SKU, record `supply_price`, `proposed_price`, `nominal_spread = proposed_price - supply_price`, the user-approved formula or the documented `traffic_default_formula`, and any omitted inputs. Under a current-run traffic-first automatic-publication authorization that expressly omits freight and/or after-sales evaluation, the default formula is `ceil(supply_price × 1.20 × 10) / 10`; it applies only when the App proves price is editable and no user formula overrides it. Require `proposed_price >= supply_price` and `nominal_spread > 0`; call this a nominal spread only, never profit, contribution, margin, or proof that the price is market-reasonable. Preserve source-synchronized or disabled prices, disable zero/unknown-stock SKUs, and do not use them to set the visible minimum price.
...
Exclude a SKU from publication when stock is zero or unknown, identity conflicts with the product, it is only an accessory/sample/deposit, it depends on an unsupported claim, or its required price is outside a viable evidenced market band. A current-turn manual traffic-price exception can waive only the last market-band condition; it cannot waive stock, identity, claim, or source-cost checks.
