...
1. First apply every user-supplied numeric opportunity predicate (for example, search count `>500` and transaction growth `>10%`) as a hard prefilter. A route is **metric-capable** only when its current App card or representative detail schema actually displays every required predicate. Never infer a missing predicate from sales, rank, a recommendation badge, or a generic category search.
2. Route A — enter `商机中心`; inspect the same-category tabs `追抖音热词` and `跟潜力爆品`, preserving recommendation reason/search count/growth data where displayed, and use each retained product's `找货源` action to collect source offers.
3. Route B — enter `商品管理`; inspect the `发布潜力商品` area and its `最近有哪些爆品推荐` entry, then use each retained product's `找货源` action to collect source offers.
4. Route C — enter `源头好货`; inspect the selection-center sections whose current App labels are exactly `抖音爆款榜` and `热搜商机`, preserving the `流量扶持` badge when shown. Treat these cards as independent supply-backed candidates rather than requiring a preceding 商机中心 record.
5. For every named route/tab, first record its literal label, category/scope, result/page count when shown, card fields, and the availability of each required user metric. Inspect one representative detail when the card schema alone cannot establish whether that metric is available. If the current schema structurally lacks any required metric, record `route_status: ineligible_by_metric_unavailable`, the exact missing metric(s), representative App evidence, and result/page count. Do not paginate that route merely to establish the absence and do not open `找货源`, create a source batch, or draft a listing from it. A route that was not inspected is never eligible for this terminal status.
6. If an attempted named entry produces no separately navigable result surface in the current App, record `route_status: unavailable_in_current_app`, the literal navigation/action evidence, and the unchanged/current surface. This terminal is allowed only after the entry was invoked and re-read; it is not a substitute for inspection.
7. Paginate or scroll every metric-capable applicable category/section in all three routes until the App exposes no further results. Deduplicate by stable product identity while preserving every route/tab/rank reference; do not stop at the first attractive item.
8. For retained Route A/B products, reconcile the `找货源` results to exact source offers. For Route C, retain the current source offer and compare same-product suppliers. Collect at least five comparable offers per verified category or opportunity cluster unless fewer genuinely exist.
9. Capture entry route, route status, metric availability, opportunity source/rank/support, product quality, review count, return rate, supplier quality, supplier return rate, on-time dispatch, support/guarantees, dispatch time, freight, restricted regions, sales, cooperating shops, stock, SKU identity, supply price, and qualifications. Preserve missing values and platform-displayed ranges literally.
10. Treat “全部优化上架” as exhaustive processing of the current eligible opportunity ledger: automatically publish every verified-category item that passes evidence, compliance, stock, supplier, claim, and conservative profit gates. Record an explicit rejection reason for every item not advanced; never publish a failing item merely to make the batch complete.
11. Apply deterministic hard stops before delegation: wrong/blocked category, unresolved qualification, unauthorized brand, all stock zero, known platform warning, or an offer whose identity cannot be reconciled across title, images, category, and SKUs.
...
3. For every included SKU, record source cost, proposed price, `nominal_spread = proposed_price - supply_price`, stock, and the formula source. If the user supplies a formula, use it. Otherwise, when the same current run explicitly authorizes traffic-first automatic publication and omission of freight/after-sales evaluation, use the deterministic `traffic_default_formula`: `proposed_price = ceil(supply_price × 1.20 × 10) / 10` (round up to the nearest CNY 0.1). Use the default only for an App-proved editable price; preserve a source-synchronized or disabled price unchanged. The proposed price must be at least the source cost and have a positive nominal spread. Disable zero/unknown-stock SKUs and never use them to create the displayed low price.
...
- Current App evidence verifies operated categories and qualifications; every named route records scope, metric availability, and either (a) pagination/scroll coverage exhausting its metric-capable applicable results or (b) an evidenced terminal `ineligible_by_metric_unavailable` / `unavailable_in_current_app`. The terminals never yield source offers or listings.
