---
name: axis-feature-detailed-design
...
2. Generate the detailed design.
   Only after `confirmed_feature` is satisfied, load `references/feature-detailed-design-template.md`, compose `$axis-tech-design-doc` patterns, and write a Chinese, evidence-backed design for one feature.
...
generated_by_skill: axis-feature-detailed-design
```
...
6. Apply composed skills.
   Use `$axis-tech-design-doc` for technical design reasoning, `$axis-db-design-doc` when persisted data is in scope, and `$axis-development-doc` for API/test/deployment document conventions. Do not broaden the output beyond one feature.
7. Verify.
...
- use `$axis-test-driven-development` only when the user asks to implement the design;
- use `$axis-oss-publish` only after explicit upload authorization and a successful dry-run.
