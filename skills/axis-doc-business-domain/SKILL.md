...
- Apply `$axis-doc-development` patterns to requirements, business flow, acceptance, API notes, and test planning.
- Apply `$axis-doc-tech-design` patterns to module boundaries, state model, transactions, errors, observability, rollout, and rollback.
- Apply `$axis-doc-db-design` patterns only when table structure, indexes, relationships, or data dictionaries are directly evidenced.
- Hand off a confirmed single feature to `$axis-doc-feature-detailed-design`; its Feature Resolution Confirmation Gate remains mandatory.
...
Report the generated path, `business_id`, inventory and architecture revisions, evidence categories, confidence boundaries, gap changes, validation commands, and residual risk. For one feature, invoke `axis-doc-feature-detailed-design` and stop for user confirmation when resolution returns zero or multiple matches.
