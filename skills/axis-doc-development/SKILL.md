...
- Every secondary detailed design makes capability-level cross-contract relationships explicit where they exist and colocates each contract's internal processing logic in its own `5.N.2`; it also contains an interface-to-code map, code-object relation map, entity/table relationship model, and end-to-end flow-to-test traceability. Implemented hops use repository-relative `path:begin-end#symbol` anchors; target or unverifiable hops are explicitly labelled. Every Section 5 interface/event/job/command is an independent sequential `5.N` group and owns all six same-prefix contract subsections; no document uses one wide interface table plus global logic/request/response/error sections.
