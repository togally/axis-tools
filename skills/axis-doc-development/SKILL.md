...
These three values are authoring and capture controls. Render them once in the overview header before the first numbered section and keep gap detail in the coverage-gap section and gap report; never generate a reader-facing `用户旅程覆盖契约` chapter.
...
- Level-1 coverage controls appear only in the overview header and gap evidence; no reader-facing `用户旅程覆盖契约` chapter is generated.
